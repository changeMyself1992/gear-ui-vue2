...
import FreePopup from '@ueip/gear-vue/lib/free-popup'
import '@ueip/gear-vue/lib/free-popup/index.css'
...
| 字段                     | 说明                                       | 类型               | 默认值    | 是否必要 |
| ---------------------- | ---------------------------------------- | ---------------- | ------ | ---- |
| v-model (value)        | 是否显示弹出层                                  | boolean          | false  | 是    |
| overlay                | 是否显示遮罩层                                  | boolean          | true   | 否    |
| position               | 弹出位置，可选值为 top bottom right left          | string           | center | 否    |
| round                  | 是否显示圆角                                   | boolean          | false  | 否    |
| lockScroll             | 是否锁定背景滚动，锁定时蒙层后面得内容无法滚动，但是嵌入弹窗得内容滚动可继续生效 | bool             | true   | 否    |
| overlay-class          | 自定义遮罩层类名                                 | string           |        | 否    |
| overlay-style          | 自定义遮罩层样式                                 | string           | object | 否    |
| duration               | 动画时长，单位秒                                 | number \| string | 0.3    | 否    |
| lazy-render            | 是否在显示弹层时才渲染节点                            | boolean          | true   | 否    |
| close-on-click-overlay | 是否在点击遮罩层后关闭                              | boolean          | true   | 否    |
| transition             | 动画类名，等价于 transition 的name属性              | string           |        | 否    |
| safe-area-inset-bottom | 是否开启底部安全区适配(兼容ios导航栏)                    | boolean          | false  | 否    |
